...
.DESCRIPTION
    ...
    - Compatível com Windows 10 1507+ e PowerShell 5.0+
.NOTES
...
function Baixar-Ferramenta ($nomeArquivo) {
    ...
    try {
        ...
        Invoke-WebRequest -Uri "$repoBase/$nomeArquivo" -OutFile $destino -TimeoutSec 15 -UseBasicParsing -ErrorAction Stop
        return $true
    }
    catch { return $false }
}
...
try {
    ...
    if ($officeName) {
        ...
        if (-not $osppFound) {
            Write-Host "   -> Verificando via WMI..." -NoNewline -ForegroundColor DarkGray
            $officeAct = Get-CimInstance SoftwareLicensingProduct -Filter "Description like '%Office%' AND PartialProductKey IS NOT NULL" -ErrorAction SilentlyContinue | Where-Object LicenseStatus -eq 1
